...
# Generated by wte. Re-run `wte hooks install` instead of editing this file.
set -u
...
def install_hooks(paths: AppPaths, force: bool = False) -> Path:
    ...
    if current and Path(os.path.expanduser(current)).resolve() != paths.hooks.resolve():
        if Path(os.path.expanduser(current)).resolve() == Path(legacy).resolve():
            ...
            raise WteError(
                f"core.hooksPath is already set to {current!r}; "
                "re-run with --force to preserve and chain it"
            )
        else:
            previous = current
