...
> is triggered.
>
> Configuration generated by older versions remains supported. The legacy keys
> `port_range`, `main_worktree`, `ports`, `secrets`, `writes`, `init`, and `skip_if`
> are aliases for the hyphenated keys shown above.
...
not process registered worktrees again, and each command's `skip-if` marker provides
